...
class DBHelper {
    ...
    func createTable(){
     let query = "CREATE TABLE IF NOT EXISTS todo(date TEXT NOT NULL, title TEXT NOT NULL, comleteOrNot INTEGER NOT NULL, PRIMARY KEY(date, title));"
        ...
        let query2 = "CREATE TABLE IF NOT EXISTS calendar(title TEXT NOT NULL, date TEXT NOT NULL, calendar_group TEXT NOT NULL);"
    ...
    func insertIntoCalendar(calendarInstance: CalendarInstance){
        //calendar insert
        let insertStatementString = "INSERT INTO calendar (title, date, calendar_group) VALUES (?, ?, ?);"
    ...
    func updateInstCalendar(calendarInstance: CalendarInstance){
        //calendar update
        let updateStatementString = "UPDATE calendar SET title = ? WHERE date = ? and title = ?;"
        var updateStatement: OpaquePointer? = nil
           // 1
           if sqlite3_prepare_v2(db, updateStatementString, -1, &updateStatement, nil) == SQLITE_OK {
               // 2
               sqlite3_bind_text(updateStatement, 1, calendarInstance.title.utf8String, -1, nil)
               sqlite3_bind_text(updateStatement, 2, calendarInstance.date.utf8String, -1, nil)
               sqlite3_bind_text(updateStatement, 3, calendarInstance.r_title.utf8String, -1, nil)
               //3
